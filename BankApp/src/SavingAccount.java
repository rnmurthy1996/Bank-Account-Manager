...
/**
 * 
 * SavingAccount.java implements the Account interface and contains the
 * variables and methods associated with a savings account.
 * 
 * @author Rohan Murthy
...
public class SavingAccount extends CheckingAccount implements Account {
	...
	/**
	 * 
	 * The secondary constructor for a savings account. This is the constructor
	 ...
	 * creation of a new account. The account number formula is 800001+number of
	...
	public int accountNumGenerator() {
		...
		// Looping to check number of saving accounts in the account list.
	...
	/**
	 * The withdraw method is called to withdraw money from the account and charges
	 * a $5 transaction fee for withdrawal. This method also adds a withdrawal
	 * transaction to the transaction list.
	 * 
	 ...
	 * another external account and charges a $2 transfer fee. This method also adds
	 * a money transfer transaction to the transaction list.
	 * 
	...
	public void moneyTransfer(Account obj, int amount) {
		...
		if (obj.getBalance() > amount) {
			...
			// withdraw fee of 5 dollar is added here to account only transfer fee.
			this.withdraw((amount - 5 + transferFee));

			// Calling deposit method to transfer money from this account to other.
			obj.deposit(amount);
		}
